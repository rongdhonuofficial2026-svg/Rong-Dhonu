...
const cp = spawn("npm.cmd", ["run", "dev"], { stdio: ["ignore", "pipe", "pipe"] });
let output = "";
cp.stdout.on("data", d => { output += d; console.log("OUT:", d.toString()); });
cp.stderr.on("data", d => { output += d; console.log("ERR:", d.toString()); });
setTimeout(() => {
