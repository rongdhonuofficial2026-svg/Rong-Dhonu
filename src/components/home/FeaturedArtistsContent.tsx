  ...
  return (
    <section className="relative bg-[#EFE6D2] overflow-hidden text-[#1E1A16]">
      ...
      <div className="absolute top-24 left-0 w-full h-px pointer-events-none"
        style={{ background: 'linear-gradient(to right, transparent, rgba(180,35,58,0.15) 30%, rgba(180,35,58,0.15) 70%, transparent)' }}
      />
      ...
      <div className="relative z-10 max-w-[1320px] mx-auto px-6 md:px-12 py-28 md:py-40">
        ...
        >
          ...
          <h2 className="font-serif text-[3rem] md:text-[5rem] lg:text-[6rem] text-[#1E1A16] font-bold leading-[1.04]">
            {title}
          </h2>
        </motion.div>
        ...
        {artists.length === 0 ? (
          ...
          >
            <p className="font-serif text-2xl text-[#1E1A16]/40 italic">
                ...
                >
                  ...
                  <Link href={`/artists/${artist.id}`} className="block w-full relative mb-6">
                    ...
                    <motion.div
                      ...
                      style={{ background: 'radial-gradient(ellipse at center, rgba(180,35,58,0.12) 0%, transparent 70%)' }}
                    />

                    <div className="relative w-full aspect-square overflow-hidden bg-[#DCCFAE]/20 border border-[#DCCFAE]"
                      style={{ boxShadow: '0 20px 50px -10px rgba(30,26,22,0.12)' }}>
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-[#1E1A16]/50 via-transparent to-transparent transition-opacity duration-700 group-hover:opacity-60" />
                      ...
                      <div className="absolute inset-0 flex flex-col justify-end p-5 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                        <span className="inline-flex items-center gap-2 bg-[#B4233A] text-white px-4 py-2 text-[10px] uppercase tracking-[0.3em] font-bold self-start">
                    ...
                    {/* Crimson corner accents on hover */}
                    <div className="absolute top-0 left-0 w-6 h-6 border-t border-l border-[#B4233A]/0 group-hover:border-[#B4233A]/80 transition-all duration-500 z-10" />
                    <div className="absolute bottom-0 right-0 w-6 h-6 border-b border-r border-[#B4233A]/0 group-hover:border-[#B4233A]/80 transition-all duration-500 z-10" />
                  </Link>

                  {/* Artist info */}
                  <div className="space-y-2">
                    <h3 className="font-serif text-xl md:text-2xl text-[#1E1A16] font-bold group-hover:text-[#B4233A] transition-colors duration-400 leading-tight">
                      <Link href={`/artists/${artist.id}`}>{name}</Link>
                    </h3>
                    <p className="text-[9px] tracking-[0.4em] uppercase text-[#5C5347] font-bold">{roleLabel}</p>
                  </div>
                </motion.div>
              )
            })}
        ...
        >
          <Link
            href="/artists"
            className="group inline-flex items-center gap-5 text-sm tracking-[0.3em] uppercase font-bold text-[#1E1A16] hover:text-[#B4233A] transition-colors duration-400"
          >
