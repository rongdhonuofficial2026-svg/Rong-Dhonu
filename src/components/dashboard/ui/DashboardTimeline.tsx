...
export function DashboardTimeline({ exhibition, locale }: DashboardTimelineProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E5E0D8] p-8 shadow-sm relative overflow-hidden">
      <div className="absolute top-0 left-0 w-1 h-full bg-accent-gold" />
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="bg-[#FAF9F6] text-[#6B655C] text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wider border border-[#E5E0D8]">
              {locale === 'bn' ? "বর্তমান প্রদর্শনী" : "Current Exhibition"}
            </span>
          </div>
          <h2 className="font-serif text-2xl font-bold text-charcoal mb-2">{title}</h2>
          <div className="flex items-center text-[#6B655C] text-sm">
        ...
        <div className="flex flex-col items-end gap-3 shrink-0">
          {daysLeft > 0 ? (
            <div className="text-right">
              ...
              <span className="text-sm text-[#6B655C] ml-2">{locale === 'bn' ? "দিন বাকি" : "days left"}</span>
            </div>
          ) : (
            <div className="text-right">
          ...
          <Button asChild variant="outline" className="rounded-full border-[#E5E0D8] text-charcoal hover:bg-[#FAF9F6]">
